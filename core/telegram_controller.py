...
if TELEGRAM_CHAT_ID:
    ...
    raise Exception("❌ TELEGRAM_CHAT_ID manquant dans .env")
...
def save_leverage(message):
    ...
    try:
        ...
        if lev < 1 or lev > 125:
            raise ValueError("Levier hors limites Binance")
...
def save_quantity(message):
    if not is_authorized(message.from_user.id):
        ...
        return
    try:
        qty_str = message.text.strip().replace(',', '.')
        qty = float(qty_str)

        if qty <= 0:
            raise ValueError("Quantité négative ou nulle")
...
def start_bot():
    ...
    bot.remove_webhook()  # <-- Ajoute cette ligne pour supprimer le webhook actif
