...
def get_dynamic_quantity():
    try:
        ...
        logging.error(f"❌ Erreur lecture quantity.txt : {e}")
...
def get_dynamic_leverage():
    try:
        ...
        logging.error(f"❌ Erreur lecture leverage.txt : {e}")
...
def retry_order(order_fn, max_retries=3, delay=2, label="ORDRE"):
    for attempt in range(max_retries):
        try:
            ...
            logging.warning("⛔ Interruption manuelle détectée. Annulation du retry.")
            raise
        except Exception as e:
            ...
            try:
                ...
                logging.warning(f"⚠️ Erreur pendant le sleep : {sleep_e}")
...
def update_status(status_text):
    try:
        ...
        logging.error(f"❌ Erreur update_status : {e}")
...
def update_trade_status_file(entry_price, status):
    try:
        ...
        logging.error(f"❌ Erreur update_trade_status_file : {e}")
...
def start_thread(target_fn, *args, **kwargs):
    ...
    try:
        ...
        logging.error(f"❌ Erreur lors du démarrage du thread {target_fn.__name__} : {e}\n{err}")
        return None
